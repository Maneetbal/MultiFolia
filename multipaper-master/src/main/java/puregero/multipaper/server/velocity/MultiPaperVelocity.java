...
public class MultiPaperVelocity {
    ...
    @Inject
    public MultiPaperVelocity(ProxyServer server, @DataDirectory Path dataFolder) {
        this.server = server;
    ...
    public void onProxyInitialization(ProxyInitializeEvent event) {
        ...
        int port = Math.toIntExact(config.getLong("port", (long) MultiPaperServer.DEFAULT_PORT));
    ...
    public void onServerConnect(ServerPreConnectEvent event) {
        RegisteredServer targetServer = event.getResult().getServer().orElseThrow();
