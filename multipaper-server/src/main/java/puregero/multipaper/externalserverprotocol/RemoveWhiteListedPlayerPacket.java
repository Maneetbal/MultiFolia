...
public class RemoveWhiteListedPlayerPacket extends ExternalServerPacket {
    ...
    public RemoveWhiteListedPlayerPacket(RegistryFriendlyByteBuf in) {
    ...
    public void handle(ExternalServerConnection connection) {
        MultiPaper.runSync(() -> {
            handlingPacket = true;
            MinecraftServer.getServer().getPlayerList().getWhiteList().remove(new NameAndId(this.uuid, this.name));
